# Food-Recipe-Hub-App-DevOps-


## Overview

The DevOps Food  Recipe App is a FullStack application built using **Express**, **Node.js**, **HTML**, **CSS**, and **MySQL**. This application allows an admin to add Recipes to a database by providing detailed information, such as the title, country, description, rating, founded year, and associated media (images and videos). The application features four main webpages:

- **Gallery**: Displays a gallery of all available Recipes.
- **Register**: Allows admins to add new recipe to the database.
- **Search**: Provides a search interface to find recipe by title or genre.
- **Watch**: Enables users to watch selected recipe.


The backend is built using Express.js and exposes an API endpoint at `/recipes` for managing movie data. Recipe media files (images and videos) are uploaded to an AWS S3 bucket using the AWS SDK.
...
1. **Fork & Clone the repository**:
    - Fork the repository https://github.com/helithjay/Food-Recipe-Hub-App-DevOps-.git
...
5. **AWS S3 Bucket**
    ...
    - Media files are stored securely in an AWS S3 bucket, Hence we need to create an AWS S3 bucket which will be further used by our application, In my case i have created an s3 bucket with the name `recipebuckethelithjay` in the eu-north-1 region, If you wanted to change the bucket name, then you can update the variable called `Bucket` in the `app.js` file
...
6. **Continuous Integration & Deployment**
    ...
    CI/CD Pipeline: The CI/CD pipeline Checks code quality using SonarQube and then it builds Docker images with tags `helijay/recipe-streaming-backend-nodejs:latest` and `helijay/recipe-streaming-frontend:latest` and pushes them to DockerHub. Now it will deploy the application using these docker images
...
- My Latest docker image for this project are present here, You can use this image as well, If you don't wanted to build your own image
    - https://hub.docker.com/r/helijay/helijay/recipe-streaming-frontend
    - https://hub.docker.com/r/jinny1/helijay/recipe-streaming-backend-nodejs
...
### Step 4: Create the EKS Cluster
  The following command creates a new EKS cluster named EKS-1 in the ap-south-1 region. No worker nodes will be created at this stage.
    ...
    eksctl create cluster --name EKS21 --region eu-north-1 --vpc-public-subnets=subnet-[subnetId],subnet-[subnetId] --nodegroup-name default-ng --node-type t3.medium --nodes=2 --nodes-min=2 --nodes-max=2 --node-volume-size=20 --ssh-access --ssh-public-key [your key pair] --managed
